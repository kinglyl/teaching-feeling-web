tyrano.plugin.kag.menu = {
    ...
    showMenu: function(call_back) {
        ...
        this.kag.html("menu", {
            ...
            layer_menu.find(".menu_skip").click(function() {
                layer_menu.html("");
                layer_menu.hide();
                if (that.kag.config.configVisible == "true") $(".button_menu").show();
    ...
    setQuickSave: function() {
        ...
        that.kag.menu.snapSave(saveTitle, function() {
            var data = that.snap;
            data.save_date = $.getNowDate() + "\u3000" + $.getNowTime();
            $.setStorage(that.kag.config.projectID +
                "_tyrano_quick_save", data)
        })
    },
    ...
    setMenu: function(j_obj) {
        ...
        var layer_menu = this.kag.layer.getMenuLayer();
        layer_menu.empty();
        var menu_html = "" + "<div class='menu_item menu_close' style='float:right;'><img src='tyrano/images/kag/menu_button_close.png' /></div>" +
    ...
    displayLog: function() {
        ...
        this.kag.html("backlog", {
            ...
            layer_menu.find(".menu_close").click(function() {
                layer_menu.hide();
                if (that.kag.config.configVisible ==
    ...
    screenFull: function() {
        if ($.userenv() == "pc" && typeof require != "undefined") {
            var gui = require("nw.gui");
            var win = gui.Window.get();
            if (win.isFullscreen) win.leaveFullscreen();
            else win.enterFullscreen()
        } else {
            alert('Fullscreen mode is not supported in current environment.');
        }
    },
    test: function() {}
};